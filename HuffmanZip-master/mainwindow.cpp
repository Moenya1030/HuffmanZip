...
{
    //设置窗口图标
    setWindowIcon(QIcon(QStringLiteral(":/new/prefix1/icon.png")));
    ...
    ui->leComAim->setStyleSheet("QLineEdit{border-width:1px;border-radius:4px;font-size:12px;color:black;border:1px solid gray;}"
                                  "QLineEdit:hover{border-width:1px;border-radius:4px;font-size:12px;color:black;border:1px solid rgb(70,225,66);}");
    ...
    ui->btnComChose->setStyleSheet("QPushButton{background-color:gray;color: white;   border-radius: 10px;  border: 2px groove gray;border-style: outset;}"
                                 ...
                                 "QPushButton:pressed{background-color:rgb(70, 225, 66); border-style: inset;}");
...
{
    ui->stackedWidget->setCurrentIndex(1);//转到压缩页
}

//主页 解压按钮
void MainWindow::on_btnUncompress_clicked()
{
    ui->stackedWidget->setCurrentIndex(2);//转到解压页
}
...
void MainWindow::on_btnComChose_clicked()
{
    ...
    if(fileDialog->exec()==QFileDialog::Accepted)
    {
        ...
        fileName.append(".huffmanTreeZip");
...
void MainWindow::on_btnComAim_clicked()
{
    ...
    //设置视图模式
    fileDialog->setViewMode(QFileDialog::Detail);
    fileDialog->setNameFilter("压缩文件(*.huffmanTreeZip)");
    //文本框显示选择的文件路径
    QString filenames;
    if(fileDialog->exec()==QFileDialog::Accepted)
    {
        filenames = fileDialog->selectedFiles().first();
        if(!filenames.endsWith(".huffmanTreeZip"))
        {
            filenames.append(".huffmanTreeZip");
        }
...
void MainWindow::on_btnComsure_clicked()
{
    ...
    progress->setFixedSize(QSize(300,66));
