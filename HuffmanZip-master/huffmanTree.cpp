#include "huffmanTree.h"
#include <bits/stdc++.h>
#include <QDebug>

HuffmanTree::HuffmanTree(int weights[])
{
    Node **treeNodes = new Node *[256];//保存字符节点
    priority_queue<Node,vector<Node>, greater<Node> > pq;//定义优先队列维护节点的权值排列
    for (int i=0, cnt=0; i<256; i++)//生成出现过的哈夫曼节点
    {
        if (weights[i] <= 0)
            continue;//若字符未出现
        treeNodes[cnt] = new Node(i, weights[i]);
        pq.push(*treeNodes[cnt++]);//入队
    }

    //数据检查
    if(pq.size()<=0)
    {
        qDebug()<<"哈夫曼节点生成错误！";
        throw runtime_error("节点生成错误！");
    }

    Node tnode1,tnode2;//用于临时存储最小两个节点的信息
    while(pq.size()>=2)//合并哈夫曼树，直到只剩一棵树
    {
        tnode1 = pq.top();
        pq.pop();
        tnode2 = pq.top();
        pq.pop();//入队
        pq.push(*mergeTree(tnode1.address, tnode2.address));
    }
    root=pq.top().address;//储存根节点地址
    makeCodeTable(root, "");//生成编码表
}

/*
合并哈夫曼树
*ltree: 左树指针
*rtree: 右树指针
return: Node* 合并后的哈夫曼树根节点地址
*/
Node* HuffmanTree::mergeTree(Node *ltree, Node *rtree)
{
    Node *parent = new Node(-1, ltree->weight + rtree->weight);
    parent->lchild = ltree;
    parent->rchild = rtree;
    ltree->parent = parent;
    rtree->parent = parent;
    return parent;
}

/*
生成哈夫曼编码表
Node *root: 当前节点指针
string code: 当前节点编码
*/
void HuffmanTree::makeCodeTable(Node *root, string code)
{
    if(root == nullptr) return;
    if(root->lchild == nullptr && root->rchild == nullptr)
    {
        codeTable.insert(map<int, string>::value_type(root->data, code));//插入
        return;
    }
    //生成时默认左边为0，右边为1
    makeCodeTable(root->lchild, code + '0');
    makeCodeTable(root->rchild, code + '1');
}
